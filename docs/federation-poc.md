# Federation PoC: joining two (or more) vortexia queues over the internet
...
- `src/federation/embeddings.js` — text embeddings, two interchangeable
...
- `src/federation/router.js` — `pickTargets(intent, directory)`: compares
...
- `src/federation/relay.js` — the shared mailbox abstraction. `InMemoryRelay`
  (used by the automated tests, `test/federation-bridge.test.js` — no
  network involved) and `GistRelay` (the real one, for this doc).
- `src/federation/bridge.js` — `FederationBridge`: per environment, listens
  for local "federation intent" broadcasts, routes them via `pickTargets`,
...
for "the internet") — see `test/federation-bridge.test.js`. This doc is for
...
```bash
node scripts/federation-demo.js env-a Clima "pronostico del tiempo, temperatura, lluvia, viento"
node scripts/federation-demo.js env-b Facturas "facturacion, factura, pagos, pagar, cobros, dinero"
node scripts/federation-demo.js env-c Meteo "clima, tiempo, temperatura, humedad, viento, pronostico"
```
...
- No authentication of *messages* — anyone with the gist id can read
  ...
  trust/signing layer discussed for real cross-machine federation
